define(function (require, exports, module) {
	...
	function bindPageEvent(){
		...
		$.preBindEvent(".parts_list ul","li",function(e){
			...
			if(productCode){
				window.location.href = "./pdDetail.html?productCode="+productCode+"&change=1";
			}
			e.stopPropagation();
		},"click");
	...
	function queryTyreList(s,num){
		...
		var paraMap = {
			"q": "supportBonusPoints:1 && publishState:1 && supportDiscount:0",
			"fl":"",
			"sort":s || "",
			"pageNum":num || ""
		}
					...
					if(result){
						...
						if(list && list.length>0){
							...
							for(var i = 0;i<list.length;i++){
								if(list[i].supportBonusPoints != "1"){
									continue;
								}
								...
								elehtml += '<p>';
								elehtml += '<strong>' + parseInt(list[i].price) + '积分</strong>';
